...
更新日期：2026-08-27。本轮在已完成的只读工具基础上实现写入、唯一替换、Diff 和受限命令执行，M1 工具层验收通过。M2 的真实 LLM、Agent Loop、工具事件回填仍未实现。
...
本轮没有新增依赖、模型调用、文件查询 API、任务取消 API 或自动 Git 提交。测试中的真实命令只运行临时工作区内的测试样例。
...
README、实施计划、结构设计文档已更新；只读阶段说明保留为历史记录，并链接到本文。
...
参数仍为 `command`（1–4000 字符）与 `timeout_seconds`（默认 30，1–120 秒）。cwd 固定为授权 Workspace 根目录，调用参数不能另设目录。采用跨平台的引号分词语法，路径建议写 `src/check.py`；有空格时加引号，不进行 Shell 变量展开。
...
拒绝任意程序名、绝对可执行文件路径、`python -c`、`node -e`、任意 Python 模块、pip/npm install、npx、cmd/PowerShell/bash 包装器，以及 `| & ; < >`、反引号、`$ % ^`、换行等 Shell 运算/展开字符；即使它们在引号内也保守拒绝。当前不支持 npm 脚本额外参数、交互输入、管道、后台常驻服务。
...
先按 README 安装后端。在仓库根目录的 Python 解释器执行下列代码；它只在新建的临时目录内写文件和运行测试，不修改 demo_workspace 或仓库源码。
...
这个例子中的测试不需要 tmp_path。测试本项目时请使用 `scripts/test.ps1`，它把 pytest 的临时目录和缓存都分离到当前账户新建的随机路径，解决此前“6 项通过，98 项 tmp_path 初始化权限失败”的问题。脚本从 backend 目录也能调用。
...
- M2/M3 待做：真实 LLM 与循环、工具结果回填、事件发布、载荷/上下文预算、Tool/Shell/File Change 专用卡片；当前任务页面不代表工具集成已完成。
