...
本文件保留 M0 历史交付记录，其中“工具未实现”等表述指 M0 时点。此后 M1–M6 已接入；当前后端全量 281 passed，M4 Demo 与 M6 多轮 smoke 均连续 3/3 成功。当前说明见 [文档导航](README.md)。
...
- `.env.example`：只描述环境变量，不包含密钥；后端启动时自动加载仓库根目录的 `.env`，且不覆盖进程中已设置的同名变量。
...
- `demo_workspace/`：此后 M4 已加入真实 Calculator Bug 和初始失败测试；本条仅保留 M0 时点历史。
...
- 本文是本次代码修改的对应说明；正式提交用 `README.txt` 现统一留在 M7，M5 为可组合 TaskRun UX，M6 为历史任务持久化与多轮会话。
