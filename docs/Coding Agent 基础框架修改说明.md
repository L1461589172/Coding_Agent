...
本文件保留 M0 历史交付记录，其中“工具未实现”等表述指 M0 时点。此后 M1 已实现全部六个本地工具、D001 已修复，M2 已实现 LLM HTTP 客户端；当前全量 221 passed，但默认 Runtime 仍未接入模型或工具循环。工具契约见 [M1 工具系统完成说明](Coding%20Agent%20M1%20工具系统完成说明.md)，客户端契约与最新验证见 [M2 说明](Coding%20Agent%20M2%20LLM%20HTTP%20适配说明.md)。
