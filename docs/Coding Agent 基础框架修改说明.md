...
本文件保留 M0 历史交付记录，其中“工具未实现”等表述指 M0 时点。当前 M1 已实现全部六个本地工具，但默认 Runtime 仍未接入它们；最新复验发现 D001，结果为 171 passed / 1 failed。工具契约和问题证据见 [M1 工具系统完成说明](Coding%20Agent%20M1%20工具系统完成说明.md)。
...
本节是历史计划，不是当前待办。文件和命令工具已实现；当前应先处理 M1 的 D001 稳定性问题，再按 [实施计划](Coding%20Agent%20实施计划.md) 接入 M2，避免重新搭建已有工具。
