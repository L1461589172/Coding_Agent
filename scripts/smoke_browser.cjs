...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '检查任务链路' }).click()
    ...
    assert.equal(await page.getByRole('button', { name: '检查任务链路' }).isEnabled(), true)
    ...
    console.log('Browser smoke passed: metadata, submit, 3 SSE events, NOT_IMPLEMENTED, mobile layout.')
  } finally {
    await browser.close()
  }
}
